...
namespace Core.Components {
	...
	public abstract class BaseComponent : MonoBehaviour, IBaseComponent {
		...
		/// 常量定义
		/// </summary>
		const string RefreshRequestName = "RefreshRequest";
		const string ClearRequestName = "ClearRequest";
		...
		void showRefreshStackTrace() {
			debugLog(requestManager.genStackTracesLog(RefreshRequestName));
		}
		...
		void showClearStackTrace() {
			debugLog(requestManager.genStackTracesLog(ClearRequestName));
		}
		...
		public virtual void requestRefresh(bool force = false) {
			if (force) refresh();
			else requestManager.request(RefreshRequestName);
		}
		...
		public virtual void requestClear(bool force = false) {
			if (force) clear();
			else requestManager.request(ClearRequestName);
		}
		...
		/// </summary>
		void resetRequests() {
			requestManager.clear(RefreshRequestName);
			requestManager.clear(ClearRequestName);
		}
		...
		protected virtual bool isRefreshRequested() {
			return shown && requestManager.isRequested(RefreshRequestName);
		}
		...
		protected virtual bool isClearRequested() {
			return requestManager.isRequested(ClearRequestName);
		}
