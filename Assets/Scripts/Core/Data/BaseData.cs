...
namespace Core.Data {
	...
	/// </summary>
	[AttributeUsage(AttributeTargets.Property)]
	public class NonAutoProp : Attribute { }
	...
	public interface IBaseData {
		...
		T convert<T>() where T : BaseData, new();
	...
	public abstract class BaseData : IBaseData {
		...
		void convertAutoAttributes(JsonData json) {
			...
			ReflectionUtils.processAttribute
				<PropertyInfo, AutoConvert>
				(GetType(), (p, attr) => {
					if (p.GetMethod == null || !attr.autoConvert) return;
			...
			ReflectionUtils.processMember
				...
				f => {
					...
					if (oriVal == null) {
						...
						var flag = ReflectionUtils.DefaultFlags;
		...
		/// <param name="keys">结果列表引用</param>
		/// <param name="oType">对象类型</param>
		/// <param name="pType">属性类型</param>
		/// <returns></returns>
		protected static List<K> fetchPropKeys<K>(
			ref List<K> keys, Type oType, Type pType) {
			if (keys == null) {
				var tmpKeys = new List<K>();

				ReflectionUtils.processMember<PropertyInfo>(
					oType, pType, p => {
						if (p.GetCustomAttribute<NonAutoProp>() == null)
							tmpKeys.Add(getKey<K>(p));
					});
		...
		/// <summary>
		/// 获取属性的Get/Set方法字典（Get => Func<>, Set => Action<>）
		/// </summary>
		/// <typeparam name="K">键类型</typeparam>
		/// <typeparam name="V">值类型</typeparam>
		/// <param name="items">结果字典引用</param>
		/// <returns></returns>
		protected Dictionary<K, Func<V>> fetchPropGetMethodDict<K, V>(
			ref Dictionary<K, Func<V>> items) {

			return fetchPropMethodDict<K, Func<V>, V>(ref items);
		}
		protected Dictionary<K, Action<V>> fetchPropSetMethodDict<K, V>(
			ref Dictionary<K, Action<V>> items) {

			return fetchPropMethodDict<K, Action<V>, V>(ref items);
		}
		Dictionary<K, D> fetchPropMethodDict<K, D, V>(
			ref Dictionary<K, D> items) where D : Delegate{

			if (items == null) {
				var tmpItems = new Dictionary<K, D>();

				ReflectionUtils.processMember<PropertyInfo, V>(
					GetType(), p => {
						if (p.GetCustomAttribute<NonAutoProp>() != null) return;

						Delegate action = null;

						if (typeof(D) == typeof(Func<V>))
							action = p.GetMethod.CreateDelegate(typeof(D), this);
						else if (typeof(D) == typeof(Action<V>))
							action = p.SetMethod.CreateDelegate(typeof(D), this);
						else return;

						tmpItems[getKey<K>(p)] = action as D;
					});
		...
		/// <param name="items">结果字典引用</param>
		/// <returns></returns>
		protected Dictionary<K, V> fetchPropValueDict<K, V>(
			ref Dictionary<K, V> items) {

			if (items == null) {
				var tmpItems = new Dictionary<K, V>();

				ReflectionUtils.processMember<PropertyInfo, V>(
					GetType(), p => {
						if (p.GetCustomAttribute<NonAutoProp>() != null) return;

						tmpItems[getKey<K>(p)] = (V)p.GetValue(this);
					});
		...
		/// <returns></returns>
		static K getKey<K>(PropertyInfo prop) {
			if (typeof(K) == typeof(string))
				return (K)(object)prop.Name;
			if (typeof(K) == typeof(Type))
				return (K)(object)prop.PropertyType;
			return default;
		}
