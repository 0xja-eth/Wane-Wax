...
namespace Core.Systems {
	...
	public class CacheItem : Attribute {
		...
		public CacheItem(string path) { this.path = path; }
	}
    ...
    public class StorageSystem : BaseSystem<StorageSystem> {
		...
		/// 初始化缓存项
		/// </summary>
		void initializeStorageItems() {
			var systems = ReflectionUtils.getNamespaceTypes(parent: typeof(BaseSystem));
			foreach (var sys in systems) processSystemCacheItem(sys);
		}
		...
		void processSystemCacheItem(Type type) {
			...
			ReflectionUtils.processAttribute<PropertyInfo, CacheItem>(
				type, (prop, cache) => {
					var path = cache.path;
					var val = prop.GetValue(sys) as BaseData;

					storageItems.Add(new StorageItem(val, path));
				});
		}
