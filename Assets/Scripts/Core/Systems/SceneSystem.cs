...
namespace Core.Systems {
	...
	public class SceneSystem : BaseSystem<SceneSystem> {
		...
		/// 场景栈
		/// </summary>
		Stack<Type> sceneStack = new Stack<Type>();
        ...
        public Type currentScene() {
            return sceneStack.Count > 0 ? sceneStack.Peek() : Type.NoneScene;
        }
        ...
        /// </summary>
        /// <returns>场景名称</returns>
        public Type realCurrentScene() {
            return (Type)SceneManager.GetActiveScene().buildIndex;
        }
        ...
        public void pushScene(Type scene, JsonData data, 
			bool reload = false, bool async = false) {
            sceneStack.Push(scene); loadScene(reload, data, async);
        }
        public void pushScene(Type scene, object data,
			bool reload = false, bool async = false) {
			var json = DataLoader.convert(data.GetType(), data);
			pushScene(scene, json, reload, async);
        }
        public void pushScene(Type scene, bool reload = false, bool async = false) {
        ...
        /// </summary>
        /// <param name="scene">场景名称</param>
        public void changeScene(Type scene, JsonData data, bool reload = false, bool async = false) {
            if (sceneStack.Count > 0) sceneStack.Pop();
            pushScene(scene, data, reload, async);
        }
        public void changeScene(Type scene, object data, bool reload = false, bool async = false) {
			var json = DataLoader.convert(data.GetType(), data);
			changeScene(scene, json, reload, async);
        }
        public void changeScene(Type scene, bool reload = false, bool async = false) {
        ...
        /// </summary>
        /// <param name="scene">场景名称</param>
        public void gotoScene(Type scene, JsonData data, bool reload = false, bool async = false) {
            clearScene(); pushScene(scene, data, reload, async);
        }
        public void gotoScene(Type scene, object data, bool reload = false, bool async = false) {
			var json = DataLoader.convert(data.GetType(), data);
			gotoScene(scene, json, reload, async);
        }
        public void gotoScene(Type scene, bool reload = false, bool async = false) {
        ...
        public void loadScene(bool reload = false, JsonData data = null, bool async = false) {
            ...
            if (scene == Type.NoneScene) gameSer.exitGame();
