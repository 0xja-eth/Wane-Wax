...
/// 属性显示类控件
/// </summary>
namespace ExerComps.Controls.ParamDisplays {

    /// <summary>
    /// 属性展示
    /// </summary>
    public abstract class ParamDisplay<T> : GeneralComponent {
    ...
    public abstract class ParamDisplay : ParamDisplay<JsonData> {
        ...
        public void activate(IDisplayDataConvertable obj,
            string type = "", bool immediately = false) {
			activate(); setValue(obj, type, immediately);
        }
        /// <param name="objs">值对象组</param>
        public void activate(IDisplayDataConvertable[] objs,
            string type = "", bool immediately = false) {
			activate(); setValue(objs, type, immediately);
        }
		...
		public virtual void setKey<T>(string key, T value, bool refresh = false) {
			if (data != null) data[key] = DataLoader.convert(value);
			requestRefresh(refresh);
		}
