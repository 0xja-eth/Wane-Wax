...
namespace ExerComps.Controls.ItemDisplays {
    ...
    public interface IItemDetailDisplay<T> : IItemDisplay<T> where T : class {
        ...
        /// 启动窗口
        /// </summary>
        void activate(T item, int index = -1);
    ...
    /// 物品详细信息，带有容器和索引功能
    /// </summary>
    public abstract class ItemDetailDisplay<T> : 
		ItemDisplay<T>, IItemDetailDisplay<T> where T : class {
        ...
        public void activate(T item, int index = -1) {
			activate(); setItem(item, index, true);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public virtual IItemDisplay<T> getItemDisplay() {
			return container?.getItemDisplay(item);
        }
